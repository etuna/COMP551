...
class FourLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=(64, 32, 32), num_classes=10,
        ...
        # Ws
        W1 = weight_scale * np.random.randn(input_dim, hidden_dim[0])
        W2 = weight_scale * np.random.randn(hidden_dim[0], hidden_dim[1])
        W3 = weight_scale * np.random.randn(hidden_dim[1], hidden_dim[2])
        W4 = weight_scale * np.random.randn(hidden_dim[2], num_classes)
        
        # bs
        b1 = np.zeros(hidden_dim[0])
        b2 = np.zeros(hidden_dim[1])
        b3 = np.zeros(hidden_dim[2])
        b4 = np.zeros(num_classes)
        
        self.params = {"W1": W1, "W2": W2, "W3": W3,"W4": W4,  "b1": b1, "b2": b2, "b3": b3, "b4": b4}
    ...
    def loss(self, X, y=None):
        ...
        alpha_dic = {'alpha': self.alpha}

        layer1, cache1 = affine_lrelu_forward(X, self.params["W1"], self.params["b1"], alpha_dic)
        layer2, cache2 = affine_lrelu_forward(layer1, self.params["W2"], self.params["b2"], alpha_dic)
        layer3, cache3 = affine_lrelu_forward(layer2, self.params["W3"], self.params["b3"], alpha_dic)
        
        scores, cache4 = affine_forward(layer2, self.params["W4"], self.params["b4"])
        ...
        loss, dx5 = softmax_loss(scores, y)
        
        loss = loss + 0.5 * self.reg *(np.sum(self.params["W1"]**2)+np.sum(self.params["W2"]**2)+ np.sum(self.params["W3"]**2)+ np.sum(self.params["W4"]**2)) 
        
        dx4, dw4, db4  = affine_backward(dx5,cache4)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers-1:
                layers[l], cache[l] = affine_forward(layers[l_prev], W, b) 
            else:
                layers[l], cache[l] = affine_lrelu_forward(layers[l_prev], W, b, alpha_dic)
                if self.use_dropout:
                    dropout_layers[l], dropout_cache[l] = dropout_forward(layers[l], self.dropout_param)
        ...
        loss, dxfinal = softmax_loss(scores, y)
        ...
        backward['layer'+str(self.num_layers)], grads['W'+str(self.num_layers)], grads['b'+str(self.num_layers)]  = affine_backward(dxfinal,cache['layer'+str(self.num_layers)])
        
        
        for i in reversed(range(self.num_layers-1)):
            if self.use_dropout:
                dx = dropout_backward(backward['layer'+str(i+2)],  dropout_cache['layer'+str(i+1)])
             
         
            backward['layer'+str(i+1)], grads['W'+str(i+1)], grads['b'+str(i+1)]  = affine_lrelu_backward(backward['layer'+str(i+2)], cache['layer'+str(i+1)])
