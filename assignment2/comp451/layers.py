...
def affine_forward(x, w, b):
    ...
    out = np.reshape(x, newshape=(x.shape[0],int(np.prod(x.shape) / x.shape[0]))).dot(w) + b
...
def sigmoid_backward(dout, cache):
    ...
    X = np.reshape(x, (x.shape[0], -1))
    dx = X.dot(1-x) #  x * (1 - x) # derivative of sigmoid
    dw = X.T.dot(dout)
    db = dout.sum(axis=0)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # Get stride & pad
    stride = conv_param['stride'] # stride = 2
    pad = conv_param['pad']  # pad = 1
    ...
    # Sliding the filter
    args = (x,w,b,stride, pad, N, H, W,F, HH, WW, H_, W_, x_pad, out)
    out = slideFilter(args)            
...
def slideFilter(args):
    x,w,b,stride, pad, N, H, W, F, HH, WW, H_, W_, x_pad, out = args
    
    for n in range(N):  # Neuron
      for f in range(F):  # Filter
        for h_ in range(H_):  # y Activation
          for w_ in range(W_):  # x Activation
            
            x_conv = x_pad[n,:,stride * h_:stride * h_ + HH,stride * w_:stride * w_ + WW]
            out[n,f,h_,w_] = np.sum(x_conv * w[f]) + b[f]
      ...
      for f in range(F):  # Filter
        for h_ in range(H_):  # y Activation
          for w_ in range(W_):  # x Activation
            ...
            x_conv = x_pad[n,:,stride * h_:stride * h_ + HH,stride * w_:stride * w_ + WW]
            dx_pad[n,:,stride * h_:stride * h_ + HH,stride * w_:stride * w_ + WW] += dout[n,f,h_,w_] * w[f]
            dw[f] += dout[n,f,h_,w_] * x_conv
...
def max_pool_forward_naive(x, pool_param):
    ...
    # H', W'
    # // --> int,   / --> float
    H_ = 1 + (H - poolHeight) // stride
    W_ = 1 + (W - poolWidth) // stride
    
    # Output tensor
    out = np.zeros(shape=(N,C,H_,W_))
    
    
    args = (out,x, N, C, H_, W_, stride, poolHeight, poolWidth)
    out = maxPoolHelper(0, args) 
...
def maxPoolHelper(op, args):
    
    if op == 0: # max_pool_forward_naive
        out,x, N, C, H_, W_, stride, poolHeight, poolWidth = args
          ...
          for c in range(C): # Channel
            for h_ in range(H_):
              for w_ in range(W_):
                    ...
                    x_pool = x[n, c, stride * h_:stride * h_ + poolHeight, stride * w_:stride * w_ + poolWidth]
                    out[n, c, h_, w_]  = np.max(x_pool)
          
       
    else: # max_pool_backward_naive
        N, C, H, W, H_, W_, poolHeight, poolWidth, dx, x, stride, dout= args
          ...
          for c in range(C):  # Channel
            for h_ in range(H_):
              for w_ in range(W_):
                ...
                x_local = x[n, c, stride * h_:stride * h_ + poolHeight, stride * w_:stride * w_ + poolWidth]
                x_bool = x_local >= np.max(x_local)
                dx[n, c, stride * h_:stride * h_ + poolHeight, stride * w_:stride * w_ + poolWidth] += dout[n, c, h_, w_] * x_bool
        out = dx
    return out
...
def max_pool_backward_naive(dout, cache):
    ...
    args = (N, C, H, W, H_, W_, poolHeight, poolWidth, dx ,x, stride, dout) 
    dx = maxPoolHelper(1, args)
