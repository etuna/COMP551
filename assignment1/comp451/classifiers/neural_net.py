...
class FourLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        L1 = np.dot(X, W1) + b1
        L1_relu = np.maximum(L1, 0)
        L2 = np.dot(L1_relu, W2) + b2
        L2_relu = np.maximum(L2, 0)
        L3 = np.dot(L2_relu, W3) + b3
        L3_relu = np.maximum(L3, 0)
        L4 = np.dot(L3_relu, W4) + b4
        scores = L4
        ...
        regularization =( np.sum(np.square(W1))*reg + np.sum(np.square(W2)))*reg + np.sum(np.square(W3))*reg 
        ...
        dW4 = L3.T.dot(softmax)
        db4 = softmax.sum(axis=0)
        dfc4 = softmax.dot(W4.T)
        
        #dW3 = softmax.dot(W4.T)
        dfc3 = dfc4.dot(W3.T)
        dW3 = dfc3.T.dot(dfc4)
        db3 = dfc4.sum(axis=0)
