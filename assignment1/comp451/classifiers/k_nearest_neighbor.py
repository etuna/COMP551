...
class KNearestNeighbor(object):
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #Cos dist  = 1 - cos similarity
                #cos_sim = np.dot(X[i],self.X_train[j].T)/(np.linalg.norm(X[i])*np.linalg.norm(self.X_train[j].T))
                #dists[i, j] = 1 - cos_sim
                dists[i, j] = 1 - np.dot(X[i], self.X_train[j].T) / (abs(X[i]).dot(abs(self.X_train[j].T)))
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #dists[i, :] = 1 - np.dot(X[i],self.X_train.T)/(np.linalg.norm(X[i])*np.linalg.norm(self.X_train.T))
            dists[i, :] = 1 - np.dot(X[i], self.X_train.T) / (abs(X[i]).dot(abs(self.X_train.T)))
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        #dists =  1 - np.dot(X,self.X_train.T)/(np.linalg.norm(X)*np.linalg.norm(self.X_train.T))
        dists = 1 - np.dot(X, self.X_train.T) / (abs(X).dot(abs(self.X_train.T)))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            counts = np.bincount(closest_y).argmax()
