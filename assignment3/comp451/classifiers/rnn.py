...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        captions_init, cache_embed = word_embedding_forward(captions_in, W_embed)
        
        # rnn
        #Use a vanilla RNN to process the sequence 
        hidden_rnn, cache_rnn = rnn_forward(captions_init, hidden_init, Wx, Wh, b)
        ...
        # loss
        #Use (temporal) softmax to compute loss 
        ...
        # rnn
        dcaptions_init, dhidden_init, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dhidden_rnn, cache_rnn)

        # word embed
        grads['W_embed'] = word_embedding_backward(dcaptions_init, cache_embed)
    ...
    def clip_grad_norm(self, grads, gclip):
        ...
        for g in grads:            
            ...
            if grad_norm > gclip:
                #gr * gc / gn
                c_grad = grad * (gclip / grad_norm)
            else:
                c_grad = grad       
            #print(c_grad)
            clipped_grads[g] = c_grad
    ...
    def sample_greedily(self, features, max_length=30):
        ...
        #initialize the hidden state of the RNN by applying the learned affine transform
        hidden_current, cache_init = affine_forward(features, W_proj, b_proj)
        ...
        for i in range(max_length):
            #Make an RNN step using the previous hidden state and the embedded current word
            hidden_current, cache_rnn = rnn_step_forward(word_embed, hidden_current, Wx, Wh, b)
            
            #Apply the learned affine transformation to the next hidden state
            scores, c = affine_forward(hidden_current, W_vocab, b_vocab)

            #Select the word with the highest score as the next word
            captions[:, i] = np.argmax(scores, axis=1)

            word_embed, cache_embed = word_embedding_forward(captions[:, i], W_embed)
    ...
    def sample_randomly(self, features, max_length=30):
        ...
        for step in range(max_length):
            ...
            captions[:, step] = sampleRandomly(probs)
...
def sampleRandomly(probs):
    return np.random.choice(range(len(probs[0])), size= (1,1), p = probs[0])
